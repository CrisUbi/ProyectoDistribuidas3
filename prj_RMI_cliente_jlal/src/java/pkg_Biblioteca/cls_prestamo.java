...
@ManagedBean()
@SessionScoped
public class cls_prestamo implements Serializable {
    ...
    public void guardar() {
        ...
        try {
            ...
            for (int i = 0; i < detalle.size(); i++) {
                LibroCuda lib = new LibroCuda();
                lib = interface1.Libro(detalle.get(i).getDetalleCudaPK().getIsbnLibro());
                cantidad1 = (detalle.get(i).getCantidad() * lib.getValorLibro().intValue()) + cantidad1;
                mensaje = interface1.modificarLibro(detalle.get(i).getDetalleCudaPK().getIsbnLibro(), lib.getCodigoAutor(), lib.getTituloLibro(), lib.getValorLibro(), (lib.getCantidadLibro() - detalle.get(i).getCantidad()));
            }
            mensaje = interface1.asiento(fechaPrestamo, "Ingreso desde Biblioteca", 0, 1, cantidad1, 0);
        } catch (RemoteException | NotBoundException ex) {
            System.out.println("error");
            mensaje = "No se pudo insertar";
        }
